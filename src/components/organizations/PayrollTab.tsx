...
export function PayrollTab({ members, payroll }: PayrollTabProps) {
  ...
  const handleVote = (memberId: string) => {
    // Use a hardcoded string value
    const user = "Jane Doe";

    setPendingMembers((prev) =>
      prev.map((member) => {
        if (member.id === memberId && !member.voters.includes(user)) {
          return {
            ...member,
            votes: member.votes + 1,
            voters: [...member.voters, user],
          };
        }
        return member;
      })
  ...
  const handleProposePayee = () => {
    ...
    const newPendingMember: PendingMember = {
      ...
      voters: [user], // The proposer is the first voter
    };
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {pendingMembers.map((member) => {
                      // Use a direct string value instead of the state variable
                      const hasVoted = member.voters.includes("Jane Doe");
