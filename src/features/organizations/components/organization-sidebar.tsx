...
export function OrganizationSidebar({ orgId }: { orgId: string }) {
  ...
  // Don't render the sidebar on mobile - it's handled by the mobile header
  if (isMobile) {
    return <SlimOrgSidebar />;
  }
  ...
  if (isLoading || !organization) {
    return (
      <>
        <SlimOrgSidebar />
        <OrganizationSidebarSkeleton isMobile={false} />
      </>
    );
  }
  ...
  return (
    <>
      ...
      <div className="w-64 h-screen flex-shrink-0 flex flex-col">
        {/* Header */}
          ...
          <nav className="">
            {OrganizationNavbarItems.map((item) => {
              if (item.value === "projects") {
                ...
                // If no projects, show a simple link that goes to the projects page
                if (!projectsData?.activeProjects?.length) {
                  return (
                    <Link
                      ...
                      className={cn(
                        "flex items-center py-3 text-sm font-medium border-b border-stone-200 px-4",
                        isProjectsActive
                ...
                // Show the accordion with projects
                return (
      ...
      <div className="border-r overflow-y-auto flex-1">
        <nav className="">
          {Array.from({ length: 5 }).map((_, index) => (
            <div
              key={index}
              className="flex items-center py-3 border-b border-stone-200 px-4"
            >
