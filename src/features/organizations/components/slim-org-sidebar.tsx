...
export function SlimOrgSidebar({ orgId, className }: SlimOrgSidebarProps) {
  ...
  return (
    <div className={cn("fixed top-0 left-0 z-30 flex h-full w-16 flex-col items-center border-r bg-background py-2 bg-stone-100 border-e-2", className)}>
      <div className="h-[50px]">
        <Link href="/" className=" flex items-center justify-center shadow-md box-shadow-md">
          <Icons.workLogo
            width={42}
            height={42}
      ...
      <ScrollArea className="flex-1 w-full border-t border-stone-200 pt-6">
        <div className="flex flex-col items-center gap-4 px-2">
          {organizations && organizations.map((org) => {
            const isActive = pathname.includes(`/organizations/${org.accountAddress}`);
                  ...
                  <TooltipTrigger asChild>
                    <Link
                      ...
                      className={cn(
                        ...
                        isActive ? "border-primary bg-primary/10" : "border-transparent hover:border-muted-foreground/20 hover:bg-muted/20"
                      )}
                    >
                      {org.logoUrl ? (
                        <Avatar className={cn("h-10 w-10 rounded-sm", isActive ? "opacity-100" : "opacity-75")}>
                          <AvatarImage src={org.logoUrl} alt={org.name} />
                          <AvatarFallback>{org.accountAddress?.charAt(0)}</AvatarFallback>
                        </Avatar>
                      ) : (
                        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-muted">
                          <span className="text-sm font-medium">{org.accountAddress?.charAt(0)}</span>
                        </div>
                      )}
                      {isActive && (
